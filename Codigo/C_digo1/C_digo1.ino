...
*/
    Serial.print(((rand() % (max + 1 - min)) + min)+45);
    Serial.print(",");
    Serial.print(((rand() % (max + 1 - min)) + min)+205);
    Serial.print(",");
    Serial.print(((rand() % (max + 1 - min)) + min)+131);
    Serial.print("\n");
    if(myTouch.dataAvailable())
    {
      myTouch.read();
      if (myTouch.getX() > x1 && myTouch.getX() < x2)
      {
        if (myTouch.getY() > y1 && myTouch.getY() < y2)
        {
          bStop = true;
        }
      }
